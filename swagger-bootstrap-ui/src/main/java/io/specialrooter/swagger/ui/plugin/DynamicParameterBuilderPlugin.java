    ...
    public void apply(ParameterContext parameterContext) {
        ...
        if (resolvedMethodParameter!=null&&resolvedMethodParameter.getParameterType()!=null&&resolvedMethodParameter.getParameterType().getErasedType()!=null){
            if (Map.class.isAssignableFrom(resolvedMethodParameter.getParameterType().getErasedType())){
                ...
                //增加组态查询注解
                Optional<RequestPageParameters> requestParametersOptional=parameterContext.getOperationContext().findAnnotation(RequestPageParameters.class);
                if (requestParametersOptional.isPresent()){
                    RequestPageParameters requestPageParameters = requestParametersOptional.get();
                    changeDynamicParameterType(requestPageParameters,parameterContext);
                }
            }
        }
    }

    private void changeDynamicParameterType(RequestPageParameters requestPageParameters, ParameterContext parameterContext){
