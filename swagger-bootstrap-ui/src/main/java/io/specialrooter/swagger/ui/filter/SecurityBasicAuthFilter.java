...
package io.specialrooter.swagger.ui.filter;

import io.specialrooter.swagger.ui.conf.Consts;
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        if (enableBasicAuth){
            if (match(servletRequest.getRequestURI())){
                //判断Session中是否存在
                Object swaggerSessionValue=servletRequest.getSession().getAttribute(Consts.SwaggerBootstrapUiBasicAuthSession);
                if (swaggerSessionValue!=null){
                    ...
                    if (upArr.length!=2){
                        ...
                        if (iptUser.equals(userName)&&iptPass.equals(password)){
                            servletRequest.getSession().setAttribute(Consts.SwaggerBootstrapUiBasicAuthSession,userName);
                            chain.doFilter(request,response);
                        }else{
                            writeForbiddenCode(httpServletResponse);
                            return;
