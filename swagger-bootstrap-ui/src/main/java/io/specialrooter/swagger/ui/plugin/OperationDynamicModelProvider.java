...
package io.specialrooter.swagger.ui.plugin;

import com.fasterxml.classmate.TypeResolver;
import io.specialrooter.swagger.ui.util.CommonUtils;
...
public class OperationDynamicModelProvider implements OperationModelsProviderPlugin {
            ...
            for (ResolvedMethodParameter parameterType : parameterTypes) {
                if (parameterType.hasParameterAnnotation(RequestBody.class)) {
                    if (Map.class.isAssignableFrom(parameterType.getParameterType().getErasedType())) {
                        ...
                        //增加组态查询注解
                        Optional<RequestPageParameters> requestParametersOptional = context.findAnnotation(RequestPageParameters.class);
                        if (requestParametersOptional.isPresent()){
                            collectDynamicParameter(requestParametersOptional.get(),context);
                        }
                    }
                }
            }
    ...
    // 增加分页查询注解
    private void collectDynamicParameter(RequestPageParameters requestPageParameters, RequestMappingContext context){
        if (requestPageParameters !=null){
            //name是否包含
            String name= requestPageParameters.name();
            ...
            Class<?> clazz= CommonUtils.createDynamicModelClass(name, requestPageParameters);
